...
import { LevelType, type Section, StatusType } from '@prisma/client';
...
async function main() {
  ...
  await prisma.courseCategory.createMany({
    data: categorySeed.map((category) => ({ ...category, id: uuidv4() })),
  });
  ...
  const generateRandomSectionData = () => {
    ...
    const sectionCount = Math.floor(Math.random() * 3) + 3; // 3-5 개
  ...
  const generateRandomLectureData = (sectionCount: number) => {
    ...
    const lectureCount = Math.floor(Math.random() * 4) + 3; // 3-6 개
    return Array.from({ length: lectureCount }, (_, index) => ({
      ...
      order: index + 1,
      duration: Math.floor(Math.random() * (1200 - 300 + 1)) + 300, // 5-20분
  ...
  const courseData = [
    ...
    level: generateRandomLevel() as LevelType,
  ...
  for (const course of courses) {
    ...
    const createdSections = [] as Section[];
...
main()
  .catch((error) => {
    console.error('시드 데이터 생성중 오류가 발생했습니다.', error);
    process.exit(1);
  })
